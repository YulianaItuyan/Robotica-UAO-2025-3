...
JOINT_COUNT = 3
GRIPPER_TOPIC = '/cmd_gripper'  # tópico para el gripper (si se usa)
...
class UpperBody(ctk.CTk):
    ...
    def gripper(self):
        ...
        if self.ros is not None:
            msg = Float32MultiArray()
            # Usamos 4 valores: los 3 ángulos + un flag (1=activar gripper)
            msg.data = [90.0, 90.0, 90.0, 1.0]  
            self.ros.publisher_.publish(msg)
            self.ros.get_logger().info("⚡ Enviado comando GRIPPER")
