...
class SerialBridgeArduino(Node):
    def __init__(self):
        ...
        self.sub_gripper = self.create_subscription(Float32MultiArray, '/cmd_gripper', self.on_cmd, 10)
...
def on_cmd(self, msg: Float32MultiArray):
    if len(msg.data) == 3:
        # Modo normal: 3 servos
        a, b, c = [self._sanitize_angle(v) for v in msg.data]
        frame = f"<{a},{b},{c}>\n"

    elif len(msg.data) == 4:
        # Modo con gripper
        a, b, c = [self._sanitize_angle(v) for v in msg.data[:3]]
        g = int(msg.data[3])   # flag del gripper
        frame = f"<{a},{b},{c},G{g}>\n"

    else:
        self.get_logger().warn(f'Esperaba 3 o 4 valores, llegaron {len(msg.data)}')
        return
