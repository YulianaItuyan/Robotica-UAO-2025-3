...
JOINT_COUNT = 6                    # ahora 6 joints: [L1,L2,L3, R1,R2,R3]
# Longitudes de eslabón (ajusta a tus medidas reales)
# Si los brazos izquierdo/derecho tienen distinta geometría, cambia a dict: LINK_LENGTHS_LEFT, LINK_LENGTHS_RIGHT
LINK_LENGTHS = [0.35, 0.20, 0.20]  # en metros (por defecto para ambos brazos)
# ====================
...
def Tx(L):
    # Mantengo tu versión (toma L como desplazamiento en Z según tu último código).
    T = np.eye(4)
    T[2,3] = L
    return T
...
class UpperBody(ctk.CTk):
    def __init__(self, ros: Ros2Bridge):
        ...
        # Inicializamos estado de todas las 6 juntas (90° = neutro)
        # Orden: [L1,L2,L3, R1,R2,R3]
        ...
        # Radio buttons para seleccionar el brazo (1=Izquierdo, 2=Derecho)
        self.arm_choice = ctk.IntVar(value=1)  # por defecto brazo Izquierdo
        ...
        self.boton_records = ctk.CTkButton(top_frame, text="RECORDS", fg_color="#737373", text_color="white",
                                           corner_radius=7, font=("Arial", 16), width=120, height=20,
                                           hover_color="#838181")
    ...
    # ======= Callbacks UI =======
    def confirmar(self):
        # Leemos los 3 ángulos del GUI (0..180)
        sliders = [float(v.get()) for v in self.slider_vars]

        # Determinamos si es izquierdo o derecho según radio button
        arm = int(self.arm_choice.get())  # 1 o 2

        # Index mapping: izquierda -> slice 0..2, derecha -> slice 3..5
        if arm == 1:
            start = 0
        else:
            start = 3
        # Actualizamos self.all_joints manteniendo la otra parte sin modificar
        for i, val in enumerate(sliders):
            self.all_joints[start + i] = val

        # Publicamos las 6 juntas (GUI envía 6 valores: L1..L3, R1..R3)
        ...
        # La FK se calcula con los 3 ángulos del brazo seleccionado
        selected_angles = self.all_joints[start:start+3]
        shifted = [a - 90.0 for a in selected_angles]
        # Usamos las longitudes definidas en LINK_LENGTHS (m)
    ...
    def stop(self):
        # Publicar posición neutra para ambos brazos
        neutral = [90.0 for _ in range(JOINT_COUNT)]
        self.all_joints = neutral.copy()
        self.ros.publish_degrees(self.all_joints)

        # Actualizar la FK para la posición neutra del brazo seleccionado
